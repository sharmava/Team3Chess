...
public class ChessController implements IChessController 
{
		...
		if(selectedPiece != null)
		{
			...
			else if(piece == null || piece.owner() != model.getActivePlayer())
			{
				if(model.validMove(column, row, selectedPiece))
				{
					try
					{
						view.removeImage(selectedPiece.column(), selectedPiece.row());
						view.setImage(column, row, selectedPiece.type(), model.getActivePlayer());
	...
	public void start() {
		...
		for(ChessPiece piece : model.getAllPieces())
			view.setImage(piece.column(), piece.row(), piece.type(), piece.owner());
	}
